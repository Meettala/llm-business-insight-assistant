...
def execute_query(df: pd.DataFrame, spec: QuerySpec) -> dict:
    ...
    if spec.operation == "count":
        if spec.group_by_column:
            result = (
                working.groupby(spec.group_by_column)
                .size()
    ...
    if spec.operation in {"sum", "mean", "min", "max"}:
        ...
        value = getattr(working[spec.value_column], aggregation)()
        return {
            "type": "scalar",
            "value": float(value),
            "operation": aggregation,
    ...
    if spec.operation == "trend":
        ...
        grouped = working.groupby(periods)[spec.value_column].sum()
        return {
            "type": "timeseries",
            "data": {str(key): float(value) for key, value in grouped.items()},
            "operation": "trend",
            "column": spec.value_column,
        }

    raise ValueError(f"Unhandled operation: {spec.operation}")
